...
class Scheduler:
    ...
    def _can_use_buffer_overlap_scoring(
        ...
        if config.prologue_fusion or config.epilogue_fusion:
            ...
            node1_template_users: OrderedSet[BaseSchedulerNode] = OrderedSet()
            for buf in node1_outputs:
                for user in buf.users:
                    if (
                        ...
                        node1_template_users.add(user.node)

            # Check if any of node1's template users also consume node2's outputs
            if node1_template_users:
                for buf in node2_outputs:
                    for user in buf.users:
                        if user.node in node1_template_users:
                            return False
